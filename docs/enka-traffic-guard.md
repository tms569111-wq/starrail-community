...
1. UID별 Enka TTL 캐시를 확인합니다.
2. TTL이 살아 있으면 `forceUpdate=true`여도 Enka를 다시 호출하지 않습니다.
3. 캐시가 없으면 JVM 전체 전역 rate limiter를 통과합니다.
4. 대기 중인 요청과 실제 동시 HTTP 요청 수를 제한합니다.
5. Enka가 HTTP 429를 반환하면 일정 시간 Enka 전체 호출을 중단하고 기존 provider fallback 로직이 MiHoMo를 시도합니다.
6. Enka/MiHoMo 장애 또는 우리 서버의 전역 Enka 혼잡 제한 때문에 요청이 실패하면 사용자 개인의 3분 쿨다운 예약은 해제합니다.
...
```text
ENKA network failure uid=*****9992 elapsedMs=8000 cause=HttpTimeoutException message=...
